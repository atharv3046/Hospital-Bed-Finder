...
async function seedHospitals() {
    ...
    // Note: Standardizing column names based on logic in List.js (bed_av_general, bed_av_oxygen, bed_av_icu)
    // Checking existing columns from previous dump: "bed_total_general", "bed_av_icu"
    // I will map them as per the database schema if I can see it exactly, 
    // but List.js used bed_av_general, bed_av_oxygen, bed_av_icu.
    ...
    const formattedHospitals = hospitals.map(h => ({
        ...
        type: h.type,
        phone: h.phone,
        bed_av_general: h.bed_av_gen,
        bed_total_general: h.bed_total_general,
        bed_av_oxygen: h.bed_av_oxygen,
        bed_total_oxygen: h.bed_total_oxygen,
        bed_av_icu: h.bed_av_icu,
        bed_total_icu: h.bed_total_icu,
        verified: h.verified,
        response_time_avg: h.response_time_avg
    }));

    const { data, error } = await supabase.from('hospitals').insert(formattedHospitals);
    if (error) {
